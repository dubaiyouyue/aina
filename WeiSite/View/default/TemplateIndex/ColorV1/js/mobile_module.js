...
(function(){
	...
	function banner(isAuto,delayTime){
		...
		if(screenWidth<($(window).width()*new_widht_baifenbi_kuandu)) screenWidth=screenWidth+1;
		
		$('.container').css('width',screenWidth);
		$('.banner li img').width(screenWidth);
		//var newsssheigtg=$('.banner li img').height();
		//alert(newsssheigtg);
		var count = $('.banner li').size();
		$('.banner ul').width(screenWidth*count);
		$('.banner ul').height(newsssheigtg);
		$('.banner').height(newsssheigtg);
		$('.banner li').width(screenWidth).height(newsssheigtg);
