...
function Myorderluyin(){
		...
		//获取最新订单号
		$mysnewww=M('dingdan')->where(array('uid'=>$user['id']))->order('id desc')->find();
		//dump($mysnewww);exit; 
		$this->assign ( 'mysnewww', $mysnewww['dingdanbianhao'] );
		
		$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/ColorV1/Myorderluyin.html' );
}
//end
...
function newtestss() {
	...
	function qiyeshichi(){
		...
		if($_POST){
			$da['gongsimingcheng']=I ( 'gongsimingcheng', 0, 'strip_tags' );
			$da['suozaiquyu']=I ( 'suozaiquyu', 0, 'strip_tags' );
			$da['gongsidizhi']=I ( 'gongsidizhi', 0, 'strip_tags' );
			$da['gongsibumen']=I ( 'gongsibumen', 0, 'strip_tags' );
			$da['lianxiren']=I ( 'lianxiren', 0, 'strip_tags' );
			$da['lianxidianhua']=I ( 'lianxidianhua', 0, 'strip_tags' );
			$da['shijiananpai']=I ( 'shijiananpai', 0, 'strip_tags' );
			$da['gongsiguimo']=I ( 'gongsiguimo', 0, 'strip_tags' );
			$da['ctime']=time();
			//dump($da);
			if(!$da['gongsimingcheng'] || !$da['suozaiquyu'] || !$da['gongsidizhi'] || !$da['gongsibumen'] || !$da['lianxiren'] || !$da['lianxidianhua'] || !$da['shijiananpai'] || !$da['gongsiguimo']) exit;
			$result = M('qiyeshichi')->add($da);
			/*if($result) {
				header('Location:/index.php?s=/addon/WeiSite/WeiSite/message.html');exit;
			}*/
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/' . $this->config ['template_index'] . '/qiyeshichiok.html' );
			exit;
			
		}
		
		$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/' . $this->config ['template_index'] . '/qiyeshichi.html' );
	...
	function shooppcaa(){
		...
		//获取商品id
		foreach($sp as $spkk=>$spvv){
			if($nowspidss) $nowspidss.=','.$spvv['sid'];
			else $nowspidss=$spvv['sid'];
			
		}
		//获取商品
		$nowspidss_mapall['id']=array('in',$nowspidss);
		//上架
		//$nowspidss_mapall['shifsxj']=1;
		//end
		$nowspidss_list_data = M ( 'custom_reply_news' )->where ( $nowspidss_mapall )->select ();
		$sfyxjsppp=0;
		foreach($nowspidss_list_data as $nowspidss_list_datak=>$nowspidss_list_datav){
			$nowspidss_list_datav['chicun']=str_ireplace("\r\n","#",$nowspidss_list_datav['chicun']);
			$chicun=explode("#",$nowspidss_list_datav['chicun']);
			//foreach($sp as $spkk=>$spvv){
				
			if($nowspidss_list_datav['shifsxj']==2){
				$nowspidss_list_data_new[$nowspidss_list_datav['id']][]='xiajia';
				//是否有下架商品
				$sfyxjsppp=1;
			}
			else $nowspidss_list_data_new[$nowspidss_list_datav['id']][]='ok';
			   foreach($chicun as $k=>$v){
					$nowspidss_list_data_new[$nowspidss_list_datav['id']][]=$v; //新库存 价格
			   }
			//}
		...
		foreach($sp as $spk=>&$spv){
			...
			foreach($nowspidss_list_data_new[$spv['sid']] as $nnkk=>$nnvv){
				if($spv['chicun']==$nnvv){
					//echo $nowspidss_list_data_new[$spv['sid']][($nnkk+1)];
					$spv['jiage']=$nowspidss_list_data_new[$spv['sid']][($nnkk+1)];
					if($nowspidss_list_data_new[$spv['sid']]['0']=='xiajia') {
						$xiajiashangpin_arr[]=$spv['title'];
						unset($sp[$spk]); //下架
					}
				}
			}
		...
		$this->assign ( 'xiajiashangpin_arr', $xiajiashangpin_arr);
		//dump($xiajiashangpin_arr);exit;
		//echo $nowspidss;
		//exit;
		//end
		...
		if(empty($sp) && $sfyxjsppp) $this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/ColorV1/shooppcaaxj.html' );
		else $this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/ColorV1/shooppcaa.html' );
	}
	//去重复元素
	...
	function SubmitOrder(){
		...
		//20170207
		if(empty($sp) && $sfyxjsppp) $this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/ColorV1/SubmitOrderxj.html' );
		else $this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/ColorV1/SubmitOrder.html' );
		//end
	...
	function delxiajia(){
		...
		foreach($sp as $spk=>&$spv){
			...
			foreach($nowspidss_list_data_new[$spv['sid']] as $nnkk=>$nnvv){
				if($spv['chicun']==$nnvv){
					...
					$spv['jiage']=$nowspidss_list_data_new[$spv['sid']][($nnkk+1)];
					if($nowspidss_list_data_new[$spv['sid']]['0']=='xiajia') {
						if(!$xiajiashangpin_arr) $xiajiashangpin_arr=$spv['sid'];
						else $xiajiashangpin_arr.=','.$spv['sid'];
						//unset($sp[$spk]); //下架
					}
				}
			}
		...
		//dump($xiajiashangpin_arr);exit;
		//删除下架商品
		$delw['sid']=array('in',$xiajiashangpin_arr);
		$delw['uid']=$user['id'];
		$delw['ok']=0;
		$dxj=M('dingdantem')->where($delw)->delete();
	...
	function fukokkk(){
		...
		if($_SESSION['randomdaofu']){
			...
			if(!$_SESSION['new_Total_fee']){
		
			header('Location:/index.php?s=/addon/WeiSite/WeiSite/Myorder/index.html');exit;
			}
			...
			//header('Location:/index.php?s=/addon/WeiSite/WeiSite/Myordernew/index.html');
			header('Location:/index.php?s=/addon/WeiSite/WeiSite/Myorderluyin/index.html');
			exit;
			return false;
		}
		...
		//余额购买
		//金额0的时候触发兑换bug
		if(!$_SESSION['new_Total_fee'] && !$_SESSION['jjppddhhhssann']){
			//header('Location:/index.php?s=/addon/WeiSite/WeiSite/SubmitOrder/index.html');
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/' . $this->config ['template_index'] . '/e.html' );
			exit;
		}
		
		if($_SESSION['gm_yue'] || (!$_SESSION['new_Total_fee'] && $_SESSION['jjppddhhhssann'])){
			$mapcyu['uid']=$user['id'];
			$syu=M('recharge')->where($mapcyu)->limit(0,1)->select();
			$rw['uid']=$user['id'];
				$viphyzjne=0;
				if($user['vip']){
					...
					//判断20161103
					//$chongzhizuidijiyueee=$this->config;
					//$chongzhizuidijineyye=$chongzhizuidijiyueee['chongzhizuidijine'];
			...
			$rd['yue']=$syu['0']['yue']-($_SESSION['new_Total_fee']-$_SESSION['gm_yhq']);
					...
					if($rd['yue']<0){
						header('Location:/index.php?s=/addon/WeiSite/WeiSite/MyAccount.html');
						exit;
					}
			...
			//if($rsa){
				...
				header('Location:/index.php?s=/addon/WeiSite/WeiSite/Myorderluyin/index.html');exit;
			//}
		}
		else{
			...
			//end
			//购买送抽奖次数
			$newssscjzssddggzzggmm=$this->config;
			$newssscjzssddggzzggmmss=$newssscjzssddggzzggmm['cjzssddggzzggmm'];
			$_SESSION['newsaaggmm']=$newssscjzssddggzzggmmss;
			//2016-12-11
	...
	function Myordernew(){
		...
		$this->assign ( 'dfkcjzssddggzzggmm', $this->config );
	...
	function gdxxddfuk(){
		...
		$user=$this->jjccddll();
			...
			//非货到付款
			if(!$_SESSION['randomdaofu'] && $_SESSION['new_Total_fee']) $data['zcj'] = array('exp','zcj+'.$nnpncjzssddggzzggmm);
			$w['id']=$user['id'];
			$ddm=M('huiyuan')->where($w)->save($data);
	...
	*/
	...
	function Hjcfssss(){
		return false;
		/*echo count(M('huiyuan')->select());
		return false;*/
		/*M('recharge')->where('yue=0')->delete(); 
		return false;*/
		$arrayre=M('huiyuan')->limit(0,1649)->select();
		foreach($arrayre as $k=>$v){
			$arr1[]=$v['tel'];
		}
		$arrayre=M('recharge')->select();
		foreach($arrayre as $k=>$v){
			$arr2[]=$v['tel'];
		...
		//$unique_arr = array_unique ( $arraynew );  
		dump(array_merge(array_diff($arr1, array_intersect($arr1, $arr2)), array_diff($arr2, array_intersect($arr1, $arr2))));//PHP里判断2个数组值是否相同 不相同的值取出来
		return false;
			//dump($arraynew);
			// 获取去掉重复数据的数组  
			$unique_arr = array_unique ( $arraynew );  
			// 获取重复数据的数组  
			$repeat_arr = array_diff_assoc ( $arraynew, $unique_arr );  
			dump($repeat_arr);
			return $repeat_arr;  
	...
	function chongzhi(){
		...
		//判断充值金额
		$chongzhizuidiji=$this->config;
		$chongzhizuidijine=$chongzhizuidiji['chongzhizuidijine'];
		
		//是否开启充值
		if($jiner && !$chongzhizuidiji['tongzhiggnryxchongzhi']){
			header('Location:/index.php?s=/addon/WeiSite/WeiSite/MyAccount.html');
			exit;
		} 
		
		if($jiner && $jiner<$chongzhizuidijine){
			header('Location:/index.php?s=/addon/WeiSite/WeiSite/MyAccount.html');
			exit;
		}
	...
	function index() {
		...
		if (file_exists ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/pigcms/Index_' . $this->config ['template_index'] . '.html' )) {
			...
			$this->_footer ();
			$nnpnnewsinxxxnew=($this->config);
if($nnpnnewsinxxxnew['tongzhikaiqixxsssxmsccdd'])
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/' . $this->config ['template_index'] . '/indexnew.html' );
		else $this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/default/TemplateIndex/' . $this->config ['template_index'] . '/index.html' );
		}
	}
