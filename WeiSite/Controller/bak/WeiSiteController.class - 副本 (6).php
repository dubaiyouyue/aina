...
class WeiSiteController extends BaseController {
	...
	function LuckDrawnew(){
		...
		if(!$_SESSION['guanzhu_choujiang']) header('Location:/index.php?s=/addon/WeiSite/WeiSite/zxcjj.html');
		...
		$s_tel=M('hcjhd')->where($mapc)->order('id desc')->limit(0,1)->select();
			...
			if(($s_tel['0']['cjbegin']>(time()+30)) || ($s_tel['0']['cjend']<(time()-30))){
				header('Location:/index.php?s=/addon/WeiSite/WeiSite/zxcjj/index.html');
			}
			//当前抽奖id
			$id=$s_tel['0']['id'];
			//$dataqc['id']=$s_tel;
	...
	function tjdaoccwwjll($leix,$texx){
		...
		$fd['ctime']=time();
		if($fd['uid'] && $fd['tel']) $faa=M('finance')->add($fd);
