...
class WeiSiteController extends BaseController {
	...
	function message(){
		...
		if($_POST){
			...
			if($result) header('Location:/index.php?s=/addon/WeiSite/WeiSite/message.html');
		}
	...
	function test(){
		...
		if(get_openid()) $_SESSION['guanzhu_choujiang']=2;
		header('Location:/index.php?s=/addon/WeiSite/WeiSite/LuckDrawnew.html');
	...
	function LuckDrawnew(){
		...
		if(!$_SESSION['guanzhu_choujiang']) header('Location:/index.php?s=/addon/WeiSite/WeiSite/LuckDraw.html');
		...
		$s_tel=M('hcjhd')->where($mapc)->order('id desc')->limit(0,1)->select();
			...
			if(($s_tel['0']['cjbegin']>(time()+30)) || ($s_tel['0']['cjend']<(time()-30))){
				header('Location:/index.php?s=/addon/WeiSite/WeiSite/LuckDraw/index.html');
			}
			//当前抽奖id
			$id=$s_tel['0']['id'];
			//$dataqc['id']=$s_tel;
		...
		$this->assign ( 'cjccsssid', $id );
			...
			/*算法*/
				...
				if($codeddd=='ok'){
					...
					$prize_arr = array(
						'0' => array('id' => 1, 'pic' => $s_tel['onepic'],'text' => $s_tel['onetext'], 'name'=>'one','num'=>$s_tel['onenum'],'centen'=>$s_tel['onecenten'],'type'=>$s_tel['onetype'],'v' => $s_tel['onegailv']*1000000),
						'1' => array('id' => 2, 'pic' => $s_tel['twopic'],'text' => $s_tel['twotext'], 'name'=>'two','num'=>$s_tel['twonum'],'centen'=>$s_tel['twocenten'],'type'=>$s_tel['twotype'],'v' => $s_tel['twogailv']*1000000),
						'2' => array('id' => 3, 'pic' => $s_tel['frepic'],'text' => $s_tel['fretext'], 'name'=>'fre','num'=>$s_tel['frenum'],'centen'=>$s_tel['frecenten'],'type'=>$s_tel['fretype'],'v' => $s_tel['fregailv']*1000000),
						'3' => array('id' => 4, 'pic' => $s_tel['foupic'],'text' => $s_tel['foutext'], 'name'=>'fou','num'=>$s_tel['founum'],'centen'=>$s_tel['foucenten'],'type'=>$s_tel['foutype'],'v' => $s_tel['fougailv']*1000000),
						'4' => array('id' => 5, 'pic' => $s_tel['fivpic'],'text' => $s_tel['fivtext'], 'name'=>'fiv','num'=>$s_tel['fivnum'],'centen'=>$s_tel['fivcenten'],'type'=>$s_tel['fivtype'],'v' => $s_tel['fivgailv']*1000000),
						'5' => array('id' => 6, 'pic' => $s_tel['sixpic'],'text' => $s_tel['sixtext'], 'name'=>'six','num'=>$s_tel['sixnum'],'centen'=>$s_tel['sixcenten'],'type'=>$s_tel['sixtype'],'v' => $s_tel['sixgailv']*1000000),
						'6' => array('id' => 7, 'pic' => $s_tel['svepic'],'text' => $s_tel['svetext'], 'name'=>'sve','num'=>$s_tel['svenum'],'centen'=>$s_tel['svecenten'],'type'=>$s_tel['svetype'],'v' => $s_tel['svegailv']*1000000),
						'7' => array('id' => 8, 'pic' => $s_tel['egipic'],'text' => $s_tel['egitext'], 'name'=>'egi','num'=>'99','centen'=>$s_tel['egicenten'],'type'=>$s_tel['egitype'],'v' => $s_tel['egigailv']*1000000),
					);
					//dump($prize_arr);
		   ...
		   if (!in_array($value, $out))
	{
	   ...
	   return $out;
		...
		$zxq=$zxq.'总数量：'.$znum.'件 总金额：'.($zjne+$nnpn).'元(运费：'.$nnpn.'元)';
		//$_SESSION['zxq']=$zxq;
	...
	function fukokkk(){
		...
		if($_SESSION['gm_yue']){
			...
			if($rsa){
					...
					$nnpnssssaaa=$nnpnssssaaa['gwdjqq'];
					if($_SESSION['new_Total_fee']>$nnpnssssaaa){
						$dzsyh_id=1;
						//$dzsyh['0']['jine']=0;
					...
					if($_SESSION['new_Total_fee']>$nnpnssssaassa){
						$dzsyh_id_ss=1;
						//$dzsyh['0']['jine']=0;
					}
				...
				//赠送优惠券
				if(!$_SESSION['gm_yhq'] && $dzsyh_id_ss) $this->zshyqqq();
				if($_SESSION['gm_yhq'] && $dzsyh_id) $this->shiyongyyqq();
	...
	function wwwffkkkid(){
		...
		if(!$wwwffkkkid) exit;//header('Location:/index.php?s=/addon/WeiSite/WeiSite/index.html');
		...
		if($ddel) header('Location:/index.php?s=/addon/WeiSite/WeiSite/SubmitOrder/index.html');
	}
	...
	*/
	...
	function ggwwwwok(){
		...
		//dump($dd);
		//dump($smupdata);
		if($supp) header('Location:/index.php?s=/addon/WeiSite/WeiSite/Myordernew/index.html');
		//echo 'fdasfads';
	}
	//更新购物车
	...
	function MyCouponsNewUsy(){
		...
		header('Location:'.$_SESSION['dangqian_url']);
		//获取我的优惠券
		//$yhq['uid']=$user['id'];
		//$yhq['ok']=0;
		//$y=M('huiyouhuiq')->where($yhq)->order('ok asc,etime desc,id desc')->select();
	...
	function shouhuodizi(){
		...
		if($_POST){
			...
			if($res) header('Location:'.$_SESSION['dangqian_url']);
	...
	function jjccddll(){
		...
		if($id && $logg){
			...
			if($s_new_logg!=$s_logg || $logg!=$s_logg || !$s_tel['0']['tel']) header('Location:/index.php?s=/addon/WeiSite/WeiSite/login.html');
	...
	function register(){
		...
		if($_POST && $_SESSION['check_code']==$_POST['telcode'] && $tel){
			...
			$data['saltr']=$saltr;
			$data['lip']=$data['rip'];
			$data['zcj']=$_SESSION['guanzhu_choujiang'];
			$data['logg']=md5($data['rip'].$data['saltr']);//md5($data['rip'].$data['mTime'].$data['saltr']);
				//注册送抽奖次数
				$nnpn=($this->config);
				$nnpn=$nnpn['cjzss'];
				//$data['zcj']=$_SESSION['guanzhu_choujiang']+$nnpn;
